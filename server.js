...
import serviceAccountKey from "./google.json" assert { type: 'json' };
    ...
    .then((user) => {
        ...
        if(!user.google_auth) {
            bcrypt.compare(password, user.personal_info.password, (err, result) => {
                if(err) {
                    return res.status(403).json({ "error": "Error occured while login please tyr again"});
                }
                if(!result) {
                    return res.status(403).json({ "error": "Incorrect passwprd"});
                } else {
...
server.post("/google-auth", async (req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        let user = await User.findOne({"personal_info.email": email}).select("personal_info.fullname personal_info.username personal_info.profil_img google_auth")
        .then((u) => {
            return u || null
        })
        .catch(err => {
    ...
    .catch(err => {
        return res.status(500).json({ "error": "Failed to authenticate you with google. Try with some other google accunt."});
    })
});
...
server.get("/trending-blogs", (req, res) =>  {
    Blog.find({ draft: false })
    .populate("author", "personal_info.profile_img personal_info.username personal_info.fullname -_id")
    .sort({ "activity.total_read": -1, "activity.total_likes": -1, "publisedAt": -1 })
...
server.post("/update-profile", verifyJWT, (req, res) => {
    ...
    if(bio.length > bioLimit){
        return res.status(403).json({ error: `Bio should not be more that ${bioLimit} characters`});
    }
...
server.post("/delete-blog", verifyJWT, (req, res) => {
    ...
    .then(blog => {
        ...
        Comment.deleteMany({ blog: blog_id }).then(data => console.log('Comment deleted'));
